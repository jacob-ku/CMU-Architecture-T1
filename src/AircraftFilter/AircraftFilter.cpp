...
bool AircraftFilter::filterAircraft(const TADS_B_Aircraft& aircraft) {
    ...
    for (const auto& [filterName, filterInstance] : filters) {
        try {
            if (!filterInstance->isAirplaneIncluded(aircraft)) {
                ...
                //          << " did not pass filter: " << filterName << std::endl;
                return false;
            } else {
                // std::cout << "AircraftFilter: Aircraft " << aircraft.HexAddr 
                //          << " passed filter: " << filterName << std::endl;
            }
    ...
    // std::cout << "AircraftFilter: Aircraft " << aircraft.HexAddr << " passed all filters" << std::endl;
    return true;
}
    ...
    if (filters.empty()) {
        ...
        for (const auto& [filterName, filterInstance] : filters) {
            ...
            if (filterInstance) {
                ...
                // 필터별 상세 정보 출력 (필요시 인터페이스에 추가 메서드 정의)
            } else {
